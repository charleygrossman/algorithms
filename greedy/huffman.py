...
def main():
    ...
    depths_merged = huffman_merge(data, depths)
    vals = sorted(list(depths_merged.values()))
    # Add one to depths since huffman_merge
    print('Maximum depth: {}, Minimum depth: {}'.format(vals[-1], vals[0]))


def huffman_merge(data, depths):
    while len(data) > 1:
        ...
        # Merge two smallest-weighted nodes and add to data. Remove the two original from data
...
# Anytime a node is seen by huffman_merge(), its depth increases by 1
